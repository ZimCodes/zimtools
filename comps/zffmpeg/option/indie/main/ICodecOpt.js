...
export default (props) => (
        <tr>
			...
			<td className={`align-middle ${props.hide}`}>
				...
				<button id={props.sector} className={"btn btn-secondary mt-3"} type="button" name="input_codec" onClick={props.resetClick}>Reset Codecs</button>
			</td>
            <td className="align-middle">
				<input id={props.id} className={props.sector} name="codec" type="text" placeholder=":a:2 ac3" onChange={props.mainTxt}/>
			</td>
            <td className={`text-center`}>1.)Select a <u>[stream type]</u>. <em><strong>:a</strong>=audio, <strong>:v</strong>=video, <strong>:s</strong>=subtitle, <strong>:d</strong>=data <strong>:t</strong>=attachment</em>. Follow by the <u>[stream index]</u><br/><br/>2.)Type a <u>[decoder]</u> to use for the stream.<br/> {CODEC_URL}|{REF_URL}</td>
        </tr>
);
