...
const OCodecOpt = props => <tr>
    <td className={'align-middle'}>{props.title}</td>
    <td className={`align-middle ${props.hide}`}>
        <input id={props.id} className={props.sector} name="codec_num" type="number" min="0" size="1" defaultValue={"0"} onChange={props.getCodecNum}/><br/>
        <button id={props.sector} className={"btn btn-secondary mt-3"} type="button" name="output_codec" onClick={props.resetClick}>Reset</button>
    </td>
